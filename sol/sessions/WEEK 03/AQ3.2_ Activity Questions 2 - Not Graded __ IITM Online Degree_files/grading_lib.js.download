...
BaseQuestion.prototype.displayFeedback = function (feedback) {
  ...
  try {
    ...
    // Silently handle convertGcbTag errors
  }
};
...
McQuestion.prototype.bind = function () {
  ...
  this.el
    .find('div.qt-check-answer > button.qt-check-answer-button')
    .click(function () {
      that.onCheckAnswer();
    });
  return this;
...
McQuestion.prototype.grade = function () {
  ...
  if (
    typeof enable_negative_marking !== 'undefined' &&
  ...
  header.append(
    $('<span>')
      ...
      .text('Score: ' + score * this.getWeight()),
  );
  ...
  if (feedback.length) {
    feedbackDiv
      .append(
        $('<h3 class="feedback-header">').text(
          this.messages.targetedFeedbackHeading,
        ),
      )
  ...
  if (this.data.defaultFeedback) {
    feedbackDiv
      .append(
        $('<h3 class="feedback-header">').text(this.messages.feedbackHeading),
      )
    ...
    for (var i = 0; i < this.data.choices.length; i++) {
      ...
      if (option.score > 0) {
        ...
        if (option.hasOwnProperty('answer_display')) {
          mc_correct_answer_div.append(
            $('<div>').append(option['answer_display']),
          );
        }
      }
    }
    feedbackDiv.append(
      $('<h3 class="feedback-header faculty-answer">').text(
        'Accepted Answers:',
      ),
    );
    feedbackDiv.append(
      $('<div class="faculty-answer">').append(mc_correct_answer_div),
    );
  }
  ...
  range_match: {
    matches: function (answer, response) {
      ...
      if (response.trim() == '') {
        ...
        response <= parseFloat(range[1]) &&
        response >= parseFloat(range[0])
      ) {
        return true;
      }
    },
...
SaQuestion.prototype.bindHintButton = function () {
  var that = this;
  this.el.find('div.qt-hint > button.qt-hint-button').click(function () {
    that.onShowHint();
  });
  return this;
};
...
SaQuestion.prototype.grade = function () {
  ...
  facultyResponse.append(
    $('<h3 class="feedback-header faculty-answer">').text('Accepted Answers:'),
  );

  var response = this.el
    .find('div.qt-response > input, div.qt-response > textarea')
    .val()
    .trim();
  for (var i = 0; i < this.data.graders.length; i++) {
    ...
    facultyResponse.append(
      $('<div class="faculty-answer">').append(answerType + grader.response),
    );

    if (
      SaQuestion.MATCHERS[grader.matcher].matches(grader.response, response)
    ) {
...
QuestionGroup.prototype.init = function () {
  var that = this;
  this.el
    .find('div.qt-mc-question.qt-embedded, div.qt-sa-question.qt-embedded')
...
function gradeScoredLesson(questions, messages, question_batch_id) {
  ...
  $('span.score').text(
    score +
      ...
      '=' +
      Math.round((score / totalWeight.toFixed(0)) * 100) +
      '%',
  );
...
function gradeAssessment(
  ...
  submitForm(graderUri || 'answer', reFetchData, showAlerts, {
...
function submitFormOld(action, hiddenData) {
  var form = $('<form/>')
    .css('display', 'none')
    .attr('method', 'post')
    .attr('action', action);
...
function submitForm(action, reFetchData = true, showAlerts = true, hiddenData) {
  // First confirm whether the logged in user is correct
  idTokenFromLocalStorage = JSON.parse(localStorage.getItem('id_token'));
  if (!idTokenFromLocalStorage) {
    return;
  }
  var userEmail = idTokenFromLocalStorage.userEmail;
  var confirmEmail =
    'You are submitting this assignment as <b>' + userEmail + '</b>';
  // if (showAlerts) {
  //   if (!confirm("You are submitting this assignment as " + userEmail)) {
  //     return;
  //   }
  // }
  ...
  if (pathname.includes('/')) {
    // mobile URL -> /courses/namespace/id
    namespace = pathname.slice(0, pathname.indexOf('/'));
  } else if (pathname.startsWith('ns_')) {
    // desktop URL -> /courses/namespace?id=id (query params are not part of pathname)
    namespace = pathname;
  } else {
    alert('Something went wrong');
    return;
  ...
  if (hiddenData.action === undefined) {
    var event = new CustomEvent('submit-assignment', {
      detail: {
        url: window.BACKEND_URL + '/' + slug + '/' + action,
        data: formData,
        confirmEmail: confirmEmail,
        reFetchData: reFetchData,
        showAlerts: showAlerts,
        action: hiddenData.action,
      },
    });
    document.dispatchEvent(event);
  }

  if (hiddenData.action === 'submit-answer') {
    var event = new CustomEvent('submit-assignment', {
      detail: {
        url: window.BACKEND_URL + '/' + slug + '/' + action,
        data: formData,
  ...
  if (hiddenData.action === 'manual-save-answer') {
    var event = new CustomEvent('manual-save-assignment', {
      detail: {
        url: window.BACKEND_URL + '/' + slug + '/' + action,
        data: formData,
        confirmEmail: confirmEmail,
  ...
  if (hiddenData.action === 'save-answer') {
    var confirmSaveEmail =
      'You are saving this assignment as <b>' + userEmail + '</b>';
    var saveEvent = new CustomEvent('save-assignment', {
      detail: {
        url: window.BACKEND_URL + '/' + slug + '/' + action,
        data: formData,
        confirmEmail: confirmSaveEmail,
        reFetchData: reFetchData,
        showAlerts: showAlerts,
        action: hiddenData.action,
      },
  ...
  //     console.log(result);
  //     endSubmitLoad('Something went wrong. Please try again')
  //   },
  //   headers: {
  //       'SEEK_ID_TOKEN': idTokenFromLocalStorage.token,
  //       'SEEK_NAMESPACE': namespace
  //   },
  //   statusCode: {
  //     200: function (xhr) {
  //       endSubmitLoad('Submission successful. You will be able to see your scores after the deadline for this assignment passes.')
  //     },
  //     403: function (xhr) {
  //       endSubmitLoad('Submission failed: deadline for this assignment has already passed.')
  //     },
  //     404: function (xhr) {
  //       endSubmitLoad('Submission failed: Looks like this assignment has been removed from the server. Please contact support if you think this is a mistake.')
  //     }
  //   }
  // })
}
  ...
  if (!$.isEmptyObject(gcbQuestions)) {
    $('div.qt-grade-scored-lesson')
      ...
      .click(function (event) {
        var dataDiv = $($(event.target).parents('[data-question-batch-id]')[0]);
    ...
    $(document).ready(function () {
      var questionBatchId = $('div.qt-grade-scored-lesson-calc-score')
        .parents('[data-question-batch-id]')
    ...
    $('button.qt-save-draft').click(function (event) {
      var dataDiv = $($(event.target).parents('[data-question-batch-id]')[0]);
      var questionBatchId = dataDiv.data('question-batch-id');
      var unitId = dataDiv.data('unit-id');
      var xsrfToken = dataDiv.data('xsrf-token');
    ...
    $('button.qt-submit-review').click(function (event) {
      var dataDiv = $($(event.target).parents('[data-question-batch-id]')[0]);
      var questionBatchId = dataDiv.data('question-batch-id');
      var unitId = dataDiv.data('unit-id');
      var xsrfToken = dataDiv.data('xsrf-token');
...
function addManualSaveButton() {
  ...
  manualSaveButton.id = 'manual-save-answer';
  manualSaveButton.textContent = '';
  manualSaveButton.style.width = '100px';
  manualSaveButton.style.overflow = 'hidden';
  manualSaveButton.style.display = 'block';
  submitButton.parentElement.appendChild(manualSaveButton);
}
!function(){try{var e="undefined"!=typeof window?window:"undefined"!=typeof global?global:"undefined"!=typeof globalThis?globalThis:"undefined"!=typeof self?self:{},n=(new e.Error).stack;n&&(e._sentryDebugIds=e._sentryDebugIds||{},e._sentryDebugIds[n]="7b19be3f-f53a-5925-900b-0e0967d27b8c")}catch(e){}}();
//# debugId=7b19be3f-f53a-5925-900b-0e0967d27b8c
